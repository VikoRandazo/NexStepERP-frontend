...
const Form: FC<FormProps> = ({ mode, fields, formikBag, selectElementEvent }) => {
  const entity = useSelector((state: StoreRootTypes) => state.ui.modal.type);
  const { data, handlers } = useForm(fields, styles, entity, selectElementEvent!);
  ...
  const mapCountries = countries.countries.map((country: string) => {
    options.push({ name: country, action: () => {} });
  }); 
